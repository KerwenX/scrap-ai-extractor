...
def test_classifier_detects_dayi_disease_page():
    html = Path("tests/fixtures/dayi_disease_sample.html").read_text(encoding="utf-8")
    classifier = PageClassifier()
    result = classifier.classify(
        ExtractionRequest(url="https://www.dayi.org.cn/symptom/123.html", raw_html=html, user_prompt="提取疾病信息"),
        build_soup(html),
    )
    assert result.site_id == "dayi"
    assert result.scenario == "disease_detail"


def test_classifier_detects_dayi_qa_page():
    html = Path("tests/fixtures/dayi_qa_sample.html").read_text(encoding="utf-8")
    classifier = PageClassifier()
    result = classifier.classify(
        ExtractionRequest(url="https://www.dayi.org.cn/qa/123.html", raw_html=html, user_prompt="提取问答内容"),
        build_soup(html),
    )
    assert result.site_id == "dayi"
